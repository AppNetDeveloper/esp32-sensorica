...
bool performFilesystemOTAUpdate(const String& filesystemUrl) {
  ...
  if (!http.begin(filesystemUrl)) {
    Serial.println("📁 [FS-OTA] ❌ No se pudo conectar al servidor");
    return false;
  }

  int httpCode = http.GET();
  if (httpCode != HTTP_CODE_OK) {
    Serial.printf("📁 [FS-OTA] ❌ Error HTTP %d\n", httpCode);
    http.end();
    return false;
  }
  ...
  int contentLength = http.getSize();
  if (contentLength <= 0) {
    Serial.println("📁 [FS-OTA] ❌ Tamaño inválido");
    http.end();
    return false;
  }
  ...
  if (!fsPartition) {
    Serial.println("📁 [FS-OTA] ❌ No se encontró partición LittleFS");
    http.end();
    return false;
  }

  Serial.printf("📁 [FS-OTA] Partición: %s (%d bytes)\n", fsPartition->label, fsPartition->size);

  if (contentLength > fsPartition->size) {
    Serial.printf("📁 [FS-OTA] ❌ Demasiado grande (%d > %d)\n", contentLength, fsPartition->size);
    http.end();
    return false;
  }
  ...
  Serial.println("📁 [FS-OTA] Escribiendo...");
  while (written < contentLength) {
    ...
    if (bytesRead > 0) {
      if (esp_partition_write(fsPartition, offset, buf, bytesRead) != ESP_OK) {
        Serial.printf("📁 [FS-OTA] ❌ Error escribiendo en offset %d\n", offset);
        http.end();
        return false;
      }
      offset += bytesRead;
  ...
  http.end();
  Serial.printf("📁 [FS-OTA] ✅ Filesystem actualizado (%d bytes)\n", written);
  return true;
}
    ...
    if (updateNeeded) {
      ...
      if (firmwareInfo.filesystemUrl.length() > 0) {
        ...
        if (performFilesystemOTAUpdate(firmwareInfo.filesystemUrl)) {
          Serial.println("📁 [FS-OTA] ✅ Filesystem actualizado correctamente");
        } else {
          Serial.println("📁 [FS-OTA] ⚠️ Error actualizando filesystem, continuando con firmware...");
        }
      }
...
void loadConfiguration() {
  ...
  mqttConfig.topic = preferences.getString("mqttTopic", "sensor/" + getDeviceShortId() + "/distance");
  ...
  deviceConfig.button1Topic = preferences.getString("button1Topic", "sensor/" + deviceId + "/button1");
...
void resetToDefaults() {
  ...
  mqttConfig.username = "";
  mqttConfig.password = "";
  mqttConfig.topic = "sensor/" + getDeviceShortId() + "/distance";
  ...
  deviceConfig.button1Topic = "sensor/" + deviceId + "/button1";
